...
The row data of the emojis table.

### Properties

| Prop          | Type     | Description                                              |
| ------------- | -------- | -------------------------------------------------------- |
| `id`\*        | `string` | Unique identifier of the emoji.                          |
| `name`\*      | `string` | Name of the emoji.                                       |
| `src`\*       | `string` | URL or source of the emoji image.                        |
| `createdBy`\* | `string` | Name of the user who added the emoji.                    |
| `createdAt`\* | `number` | Creation time of the emoji. Formatted as timestamp (ms). |
...
```ts
{
  id: "e79a0b74-3aba-4149-9f74-0bb5791a6ee6",
  name: "party-parrot",
  src: "https://storage.example.com/emojis/party-parrot.gif",
  createdBy: "Steve",
  createdAt: 1738368000000
}
```
...
| `billedTo`        | `string`  | Business name / billed to name. |
| `billingEmail`    | `string`  | Billing email address.          |
| `invoiceEmails`   | `boolean` | Whether invoice emails are on.  |
| `vatNumber`       | `string`  | VAT / tax ID number.            |
| `upcomingInvoice` | `string`  | Upcoming invoice description.   |
...
| `address`\* | `object` | Address details (below). |
