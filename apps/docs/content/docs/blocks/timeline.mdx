...
Pass `anchorDate` and `onAnchorDateChange` to keep the currently viewed date
outside the timeline, or use `defaultAnchorDate` for an initial position.
The timestamp describes the center of the viewport after the sidebar width
is removed. Scroll updates are reported before a layout can unmount; they
can be stored separately from persisted view settings.

`timeZone` controls date boundaries and labels. Invalid zones fall back to
UTC. `startDate` and `endDate` still bound the timeline data. An anchor outside
the scrollable range is clamped and its actual visible date is reported.
...
| `anchorDate` | `number` | - | Controlled center timestamp. |
| `defaultAnchorDate` | `number` | Today | Initial center timestamp. |
| `onAnchorDateChange` | `(date: number) => void` | - | Receives the viewed timestamp after user scrolling. |
| `timeZone` | `string` | Browser zone | IANA timezone used to display dates. |
