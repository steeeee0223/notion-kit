...
| `toReadableValue`\*   | `(data: Data, row: Row) => string`                                | Converts the plugin's data type to a readable value. This is useful for sorting a column.                              |
| `toTextValue`\*       | `(data: Data, row: Row) => string`                                | Converts the plugin's data type to a text value. This is useful for displaying/exporting the data in a compact format. |
