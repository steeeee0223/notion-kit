...
- `timeline-property`: Timeline by selects a visible, undeleted property with plugin ID `date`; generated Created time/Last edited time properties are ineligible. If none is usable, initialization creates an editable Date property, seeds row dates from row timestamps, and selects it. Related resource events share an operation ID. [Property selection](../../../../packages/table-view/src/menus/layout-menu.tsx), [initialization](../../../../packages/table-view/src/timeline-view/use-timeline-view-state.ts), [date adaptation](../../../../packages/table-view/src/timeline-view/timeline-adapter.ts).
...
- `layout-plugin-behavior`: layouts share the hook's plugin-derived sorting/grouping and UI group-label renderers. List/Board omit many empty compact values but retain unchecked Checkbox; Timeline renders title plus its selected date track and exposes other properties in row detail. Generated times remain read-only while supporting date sorting/grouping/filtering/calculation. [Plugin matrix](plugin-behaviors.md), [group rows](../../../../packages/table-view/src/table-body/table-grouped-row.tsx), [Board headings](../../../../packages/table-view/src/board-view/board-group.tsx).
...
2. Locate the **Timeline table** sidebar title with `table.timelineSidebarRow("row-alpha").getByLabel("Alpha", { exact: true })`. Read its bounding box, then click inside its trailing edge at `{ x: width - 12, y: height / 2 }`. Require Alpha detail and `openedRowId: "row-alpha"`; close it. Report any overlapping row controls as a product issue; this check does not prove that the whole title area is usable.
...
- Timeline's grouping property and Timeline by date property are independent. Table/List/Timeline share collapsible group rows; Board uses column headings. Group selection is exposed in Table/List, not Timeline.
