...
- `edit-commit`: text/title/link popovers commit on Enter or blur; Escape discards the draft. Number editing converts valid input to a normalized numeric string and empty/invalid input to null. [Text input](../../../../packages/table-view/src/common/text-input-popover.tsx), [number editor](../../../../packages/table-view/src/plugins/number/number-cell.tsx).
...
- `plugin-consumer`: table-hook data plugins define defaults, conversion, text matching, and methods; table-view pairs each data plugin ID with exactly one UI adapter. Duplicate IDs, missing adapters, and orphan adapters are rejected. [Data plugin contract](../../../../packages/table-hook/src/plugins/types.ts), [UI registry](../../../../packages/table-view/src/plugins/registry.ts). Type/configuration menus derive their available capabilities from registered plugins.
