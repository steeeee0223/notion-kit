...
- `sort-rules`: multiple prioritized properties, direction/method choice, property replacement, removal, and drag priority. The same property cannot be added twice. A header sort replaces the whole rule list; empty numeric values sort last. [Sort editor](../../../../packages/table-view/src/menus/sort-menu.tsx), [header sorting](../../../../packages/table-view/src/menus/prop-menu.tsx), [sorting resolution](../../../../packages/table-hook/src/methods.ts).
...
- Search, basic grouping, sort rules, and counting use internal table state. Structured filters write `view.filters`; grouping-method and group-sort selections also emit view resource events. Do not equate all grouping state with an unchanged view resource.
- Group using appears only for plugins with multiple grouping methods. Automatic group sorting and manual drag order are different contracts.
