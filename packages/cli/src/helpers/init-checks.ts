...
export async function initChecks(options: Options) {
  ...
  const exit = (message: string, error = true) => {
  ...
  const framework = await getProjectFramework(options.dir);
  if (!framework) {
    exit(
  ...
  const reactVersion = getReactVersion(options.dir);
  if (!reactVersion) {
    exit(
  ...
  const tailwindVersion = getTailwindVersion(options.dir);
  if (!tailwindVersion) {
    exit(
